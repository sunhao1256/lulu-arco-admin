...
export function getBreadcrumbByRouteKey(
  ...
  const breadcrumbMenu = getBreadcrumbMenu(
    (menu) => activeKey.includes(menu.routeName),
    menus
  );
...
export function getBreadcrumbsByPredicate(
  predicate: (menu: App.GlobalMenuOption) => boolean,
  menus: App.GlobalMenuOption[],
  rootPath: string
) {
...
function getBreadcrumbMenuItem(
  ...
  if (predicate(menu) && menu.children && menu.children.length) {
    breadcrumbMenu.push(
      ...menu.children
        .map((item) =>
          getBreadcrumbMenuItem(predicate, item as App.GlobalMenuOption)
        )
        .flat(1)
    );
  }
